...
/**
 * Class hzHaroldasZapalskisModule
 * Handles module setup, provides additional tools and module related helpers.
 *
 * @codeCoverageIgnore
 */
class hzHaroldasZapalskisModule extends oxModule
{
    ...
    {
        $sModuleId = 'hzharoldaszapalskis';

        $this->setModuleData(
            array(
                 'id'          => $sModuleId,
                 'title'       => 'HZ Haroldas Zapalskis',
                 'description' => 'HZ Haroldas Zapalskis Module',
            )
        );

        $this->load($sModuleId);

        oxRegistry::set('hzHaroldasZapalskisModule', $this);
    }
    ...
    public function translate($sCode, $blUseModulePrefix = true)
    {
        if ($blUseModulePrefix) {
            $sCode = 'HZ_HAROLDASZAPALSKIS_' . $sCode;
        }
    ...
    {
        if ($blUseModulePrefix) {
            $sModuleSettingName = 'hzHaroldasZapalskis' . (string) $sModuleSettingName;
        }
    ...
    {
        return oxRegistry::getConfig()->getModulesDir() . 'hz/haroldaszapalskis/';
    }
